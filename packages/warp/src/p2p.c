...
size_t volunteer_used_bytes(void) {
    DIR *d = opendir(WARP_VOLUNTEER_DIR);
    if (!d) return 0;
    size_t total = 0;
    struct dirent *ent;
    while ((ent = readdir(d))) {
        ...
        char path[600];
        snprintf(path, sizeof(path), "%s/%s", WARP_VOLUNTEER_DIR, ent->d_name);
...
static int volunteer_has_pkg(const char *sha256) {
    char path[600];
    snprintf(path, sizeof(path), "%s/%s.warp", WARP_VOLUNTEER_DIR, sha256);
...
static void volunteer_write_meta(const char *sha256, const char *name,
                                  const char *version) {
    char meta[600];
    snprintf(meta, sizeof(meta), "%s/%s.meta", WARP_VOLUNTEER_DIR, sha256);
    FILE *f = fopen(meta, "w");
    if (!f) return;
...
static int build_announce_packages(char *buf, size_t bufsz) {
    ...
    DIR *vd = opendir(WARP_VOLUNTEER_DIR);
    if (vd) {
        while ((ent = readdir(vd)) != NULL) {
            ...
            snprintf(meta_file, sizeof(meta_file),
                     "%s/%s.meta", WARP_VOLUNTEER_DIR, sha);
            char pkg_name[WARP_MAX_NAME] = "";
            size_t mlen;
...
int p2p_announce(const char *announce_url, const char *pkg_name,
    ...
    snprintf(body, sizeof(body),
             "{\"port\":%d,\"packages\":%s}", port, packages);
...
static char *build_pkg_list_json(void) {
    ...
    if (vd) {
        while ((ent = readdir(vd)) != NULL) {
            ...
            snprintf(warp_file, sizeof(warp_file),
                     "%s/%s", WARP_VOLUNTEER_DIR, ent->d_name);
...
int p2p_volunteer(warp_seed_config_t *cfg, int port) {
    mkdirs(WARP_VOLUNTEER_DIR, 0755);
    ...
    for (int i = 0; i < idx.count; i++) {
        ...
        snprintf(dest, sizeof(dest), "%s/%s.warp", WARP_VOLUNTEER_DIR, e->sha256);
    ...
    if (cfg->serve) {
        ...
        p2p_announce(WARP_TRACKER_URL, NULL, NULL, port);
    }
